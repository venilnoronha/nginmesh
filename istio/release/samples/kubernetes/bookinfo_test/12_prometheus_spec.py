...
with description('nginmesh Test 12'):
    with before.all:
         #Read Config file
         configuration.setenv(self)

    with context('Set environment'):
         with it('Add Prometheus tracing feature'):
    ...
    with context('Starting Test'):
        with it('Bookinfo Prometheus feature'):
            for _ in range(5):
                r = requests.get(self.url)
                r.status_code
                expect(r.status_code).to(equal(200))
            r1=requests.get(self.prometheus)
            r1.status_code
            expect(r1.status_code).to(equal(200))
            if 'istio-system' in r1.text:
                expect(0).to(equal(0))
            else:
    ...
    with context('Clean Environment'):
        with it('Delete Prometheus feature'):
